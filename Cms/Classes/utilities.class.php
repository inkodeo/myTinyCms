...
class utilities {
     ...
     */ {
        ...
        // ceci est un exemple de mauvais code de service, il ne fonctionne que pour un nombre limit� de structure de crop
        // c'est donc un code m�tier � surcharger.
        ...
        /* 	DEPRECIEE - Malheureusement, ce code ne marche une fois appel�e par getNodes 
          for($i=0; $i<count($pNodeSet); $i++)
          {
          $_content = $pNodeSet[$i]->childNodes ;

          // cette fonction ne descend qu'� un niveau
        ...
        foreach ($_users as $u) {
            if ($u->nodeType != XML_TEXT_NODE) {
                ...
                foreach ($pQueries as $_qa) {
                    if (($u->getAttribute($_qa['attribute']) == trim($_qa['value']))) { // 1:1 match
                        ++$j;
                    } else {
                        --$j;
                    }
